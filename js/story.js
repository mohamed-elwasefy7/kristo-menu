...
/* ---------- 2b. Daily board — the week's rotating plates ---------- */
export function buildDailyBoard(daily) {
  if (!daily?.days?.length) return null;
  const s = section("daily-board", "paper", "screenDaily", "daily");

  // Only the plates being served right now. On a closed day (or one with no
  // list) the board rolls forward to the next serving day and says so, so the
  // screen is never empty and never advertises food the kitchen isn't cooking.
  const today = new Date().getDay();
  const days = daily.days;
  let day = days.find((d) => d.weekday === today && d.dishes?.length);
  const isToday = Boolean(day);
  if (!day) {
    const ahead = [...days]
      .filter((d) => d.dishes?.length)
      .sort((a, b) => ((a.weekday - today + 7) % 7) - ((b.weekday - today + 7) % 7));
    day = ahead[0];
  }
  if (!day) return null;

  const rows = day.dishes.map((dish) => {
    const accent = getLang() === "ar" ? dish.name.en : dish.name.ar;
    const dir = getLang() === "ar" ? 'lang="en" dir="ltr"' : 'lang="ar" dir="rtl"';
    return `<li class="daily__item">
  ...
  s.innerHTML = `
    ...
    <div class="narrative__content narrative__content--center daily__head">
      ...
      ${isToday
        ? `<p class="daily__day-line" data-enter="rise" data-enter-at="0.18">
             <span class="daily__day-name">${t(day.label)}</span>
             <em class="daily__today">${t("todayLabel")}</em>
           </p>`
        : `<p class="daily__day-line daily__day-line--next" data-enter="rise" data-enter-at="0.18">
             <span class="daily__day-name">${t({ ar: "القائمة القادمة", en: "Serving next" })}</span>
           </p>`}
      ${daily.price ? `<p class="daily__price" data-enter="rise" data-enter-at="0.24">${t("dailyAllFor")} <strong>${daily.price}</strong> ${t("sar")}</p>` : ""}
    </div>
    <ul class="daily__list" role="list" data-enter="rise" data-enter-at="0.34">${rows}</ul>`;
  return s;
}
...
export function buildClosing(restaurant) {
  ...
  const orderBtns = Object.entries(restaurant.orderLinks || {})
    ...
    .map(([app, url]) =>
      `<a class="btn btn--primary" href="${url}" target="_blank" rel="noopener">${appLabel(app)}</a>`)
    .join("");
  s.innerHTML = `
