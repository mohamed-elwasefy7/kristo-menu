...
export function buildDailyBoard(daily) {
  ...
  const openIndex = Math.max(0, daily.days.findIndex((d) => d.weekday === today));
  ...
  s.innerHTML = `
    ...
    <ul class="daily__list" role="list" data-enter="rise" data-enter-at="0.4"></ul>`;

  const tabs = $(".daily__days", s);
  const list = $(".daily__list", s);

  const paint = (i) => {
    $$("button", tabs).forEach((b, bi) => b.setAttribute("aria-selected", String(bi === i)));
    list.innerHTML = "";
    daily.days[i].dishes.forEach((dish) => {
      const li = document.createElement("li");
      li.className = "daily__item";
      const accent = getLang() === "ar" ? dish.name.en : dish.name.ar;
      li.innerHTML =
        `<span class="daily__name">${t(dish.name)}</span>` +
        (accent ? `<span class="daily__alt" ${getLang() === "ar" ? 'lang="en" dir="ltr"' : 'lang="ar" dir="rtl"'}>${accent}</span>` : "");
      list.append(li);
    });
  };
  ...
  daily.days.forEach((day, i) => {
    ...
    b.type = "button";
    b.role = "tab";
    b.className = "daily__day";
    b.setAttribute("aria-selected", "false");
    b.innerHTML = `<span>${t(day.label)}</span>${day.weekday === today ? `<em class="daily__today">${t("todayLabel")}</em>` : ""}`;
