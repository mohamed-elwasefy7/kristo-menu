...
export function initI18n(strings) {
  uiStrings = strings;
  // ?lang=ar|en wins (lets the printed QR code pin the language), then the
  // saved choice, then the device language
...
/* set src (+srcset when available) on an <img> from a basename.
   Mobile framing is handled per-dish via `mobile.objectPosition` (CSS
   object-position), so no separate mobile-crop files are probed — that keeps
   the console free of 404s from speculative "{basename}-m.*" lookups. */
