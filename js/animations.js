/* كريستو — animations.js
   ...
   lazily on first approach; IO plays/resets with a fast-swipe defer. */

import { $, $$, on, prefersReducedMotion } from "./utils.js";
import { upgradeFloats } from "./ingredients.js";

const MOODS = ["paper", "mist", "turquoise", "teal-deep", "ember", "ink"];
const THEME_COLORS = {
  paper: "#F4EFE6", mist: "#B6C9C6", turquoise: "#6FA29F",
  "teal-deep": "#4C837F", ember: "#5E2320", ink: "#16211F",
};
...
let heroLoops = [];
let motionSuspended = document.visibilityState === "hidden";
let visibilityBound = false;

export function init() {
  reduced = prefersReducedMotion();
  on("dish:active", ({ mood, section }) => {
  ...
  if (!visibilityBound) {
    ...
    document.addEventListener("visibilitychange", () => {
      motionSuspended = document.visibilityState === "hidden";
      document.body.classList.toggle("is-tab-hidden", motionSuspended);
      if (motionSuspended && window.gsap) {
        gsap.killTweensOf($$(".atmo-layer"));
        ambients.forEach((tweens) => tweens.forEach((tween) => tween.pause()));
      }
      syncLoopState();
    });
  }
...
function syncLoopState() {
  const heroActive = activeSection?.classList.contains("hero") || $(".hero")?.classList.contains("is-active-screen");
  const shouldRunHero = !reduced && !motionSuspended && heroActive;
  heroLoops.forEach((tween) => shouldRunHero ? tween.resume() : tween.pause());

  if (!motionSuspended && activeSection) {
    (ambients.get(activeSection) || []).forEach((tween) => tween.resume());
  }
}

/* ---------- atmosphere ----------
   currentMood starts null so the very first dish:active always applies —
   the loader's turquoise theme-color needs handing over to the hero mood */
let currentMood = null;

function setMood(mood) {
  if (!MOODS.includes(mood)) mood = "paper";
...
export function bindDishes() {
  ...
  const indexOf = new Map(sections.map((s, i) => [s, i]));
  sections.forEach((section) => section.classList.add("anim-ready"));

  // ~100 screens: building every timeline eagerly would cost one long idle
  // burst, so each section's timeline is built at its first 15% crossing —
  // one screen ahead of the 60% play threshold.
  const ensure = (section) => {
    if (!timelines.has(section)) {
      ...
      ambients.set(section, buildAmbient(section, indexOf.get(section) || 0));
    }
    return timelines.get(section);
  };
    ...
    (entries) => {
      for (const entry of entries) {
        ...
        if (entry.intersectionRatio >= 0.6) {
          ...
          ensure(section); // approaching — pre-build so the play is instant
        } else {
          clearTimeout(playTimers.get(section));
          ...
          tl.pause(0);
          resets.get(section)?.();
          (ambients.get(section) || []).forEach((t2) => t2.pause());
...
function buildDishTimeline(section) {
  ...
  add(".dish__name .reveal__inner", { yPercent: 125, opacity: 1 }, { yPercent: 0, duration: 0.8, ease: "power4.out" }, 0.06);
...
const ENTER_RECIPES = {
  ...
  mask: [{ yPercent: 125, opacity: 1 }, { yPercent: 0, duration: 0.9, ease: "power4.out" }],
...
export function heroEntrance() {
  if (!window.gsap || reduced) return;
  heroLoops.forEach((tween) => tween.kill());
  heroLoops = [];
  const hero = $(".hero");
  ...
  if (light) {
    ...
    heroLoops.push(gsap.to(light, {
      xPercent: 24, duration: 12, ease: "sine.inOut", yoyo: true, repeat: -1, delay: 1.6, paused: true,
    }));
  }
  ...
  add(".hero__brandmark", { scale: 0.9, opacity: 0, rotate: -4 },
    { scale: 1, opacity: 1, rotate: -2, duration: 0.65, ease: "back.out(1.25)" }, 0.28);
  add(".hero__eyebrow", { y: 10, opacity: 0 }, { y: 0, opacity: 1, duration: 0.5 }, 0.3);
  add(".wordmark--hero-v2 .reveal__inner", { yPercent: 125 },
  ...
  add(".hero__tagline .reveal__inner", { yPercent: 125 }, { yPercent: 0, duration: 0.7 }, 0.8);
  ...
  if (breathe) {
    heroLoops.push(gsap.to(breathe, {
      scale: 1.025, duration: 9, ease: "sine.inOut", yoyo: true, repeat: -1, paused: true,
    }));
  }
  // floating logo — starts once the entrance settles
  const mark = $(".wordmark--hero-v2", hero);
  if (mark) {
    heroLoops.push(gsap.to(mark, {
      y: -4, duration: 7, ease: "sine.inOut", yoyo: true, repeat: -1, delay: 1.6, paused: true,
    }));
  }
  syncLoopState();
}
