...
export function init() {
  ...
  on("dish:active", ({ mood }) => setMood(mood || "paper"));
}
...
/* ---------- hero entrance — logo settles, section tiles cascade in ---------- */
export function heroEntrance() {
  ...
  // scale-only: the loader's dissolve provides the fade, and keeping the logo
  // painted from the start lets it register as LCP at first paint
  add(".hero__logo", { scale: 1.08 }, { scale: 1, duration: 1.0, ease: "power2.out" }, 0.05);
  add(".hero__cats > *", { y: 12, scale: 0.94, opacity: 0 },
    { y: 0, scale: 1, opacity: 1, duration: 0.5, stagger: 0.05, ease: "back.out(1.3)" }, 0.45);
  add(".swipe-hint", { opacity: 0 }, { opacity: 1, duration: 0.6, ease: "power2.out" }, 1.2);

  const breathe = $(".hero__breathe", hero);
  if (breathe) {
    gsap.to(breathe, { scale: 1.035, duration: 9, ease: "sine.inOut", yoyo: true, repeat: -1 });
  }
}
