...
import { $, emit, prefersReducedMotion, applyImage } from "./utils.js";
...
export async function run(menuPromise, storyPromise = Promise.resolve()) {
  ...
  const tasks = [
    loadCriticalFonts().then(() => bump(0.3)).catch(() => bump(0.3)),
...
function loadCriticalFonts() {
  ...
  return Promise.all([
    document.fonts.load('700 1em "Aref Ruqaa"', "كريستو"),
    document.fonts.load('400 1em "Tajawal"', "أهلاً بكم"),
  ]);
}
...
async function loadHero() {
  ...
  // repair path: re-apply the deterministic AVIF/WebP/JPEG picture contract
  await applyImage(img, "hero", "assets/hero/", "(min-width:1024px) 48vw, 86vw");
