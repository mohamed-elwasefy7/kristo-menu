...
import {
  $, $$, emit, t, getLang, storage, toast, ICONS, appLabel,
} from "./utils.js";
...
export function render() {
  ...
  (data.categories || []).forEach((cat) => {
    ...
    // alwaysShow categories (e.g. the weekly daily-dishes board) keep their
  ...
  if (orphans.length) {
    ...
    orphans.forEach((dish) => append(buildDish(dish, tpl, catLabel, favs), "end"));
  }
...
function renderHeroCategories() {
  ...
  (data.categories || []).forEach((cat) => {
    ...
    tile.type = "button";
    tile.className = cat.alwaysShow ? "hero__cat hero__cat--daily" : "hero__cat";
