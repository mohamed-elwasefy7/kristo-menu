/* كريستو — menu.js
   ...
   hero → welcome → [category intro → dishes] × categories →
   drinks → order → closing.
   Everything reads from data; language re-render rebuilds it all. */

import {
  $, $$, emit, t, getLang, storage, toast, ICONS, appLabel, prefersReducedMotion,
} from "./utils.js";
import * as story from "./story.js";
import { mountFloats } from "./ingredients.js";

let data = null;
// price is intentionally NOT required — null/missing price just hides the price line
const REQUIRED = ["id", "name", "image", "background"];
...
async function fetchJSON(path, optional = false) {
  try {
    ...
    if (optional) {
      console.warn(`[kristo] ${path} unavailable`, err);
      return null;
    }
    throw err;
  }
...
export async function render() {
  ...
  let batchSize = 0;
  const yieldAfterBatch = async () => {
    batchSize += 1;
    if (batchSize % 10 !== 0) return;
  ...
  if (st) {
    append(story.buildWelcome(), "welcome");
  ...
  // dishes whose category id matches no entry in categories.json still render
  const orphans = data.dishes.filter((d) => !seen.has(d.id));
  if (orphans.length) {
    console.warn("[kristo] dishes without a known category:", orphans.map((d) => d.id));
    for (const dish of orphans) {
      append(buildDish(dish, tpl, catLabel, favs), "end");
      await yieldAfterBatch();
    }
  }

  if (data.drinks.length) {
    append(renderDrinks(), "drinks");
    await yieldAfterBatch();
  }

  if (st) {
    append(story.buildOrder(data.restaurant || {}), "end");
    append(story.buildClosing(data.restaurant || {}), "end");
  }

  // Commit the complete menu in one DOM operation. During a language change,
  // the current screens stay intact until their replacement is ready.
...
function renderHeroCategories() {
  ...
  if (!wrap) return;
  wrap.innerHTML = "";
  const indexWrap = $("#menu-index-links");
  if (indexWrap) {
    indexWrap.innerHTML = "";
    indexWrap.setAttribute("aria-label", t("menuIndex"));
  }
  const hasDishes = (catId) => data.dishes.some((d) => d.category === catId);

  const addIndexLink = (label, target) => {
    if (!indexWrap) return;
    const button = document.createElement("button");
    button.type = "button";
    button.className = "menu-index__link";
    button.dataset.goto = target;
    button.textContent = label;
    indexWrap.append(button);
  };

  (data.categories || []).forEach((cat) => {
    if (!hasDishes(cat.id)) return;
    const chip = document.createElement("button");
    chip.type = "button";
    chip.className = "hero__cat";
    chip.dataset.goto = `cat-${cat.id}`;
    chip.textContent = t(cat.label);
    wrap.append(chip);
    addIndexLink(t(cat.label), `cat-${cat.id}`);
  });
  if (data.drinks.length) {
    const chip = document.createElement("button");
    chip.type = "button";
    ...
    wrap.append(chip);
    addIndexLink(t("drinksTitle"), "drinks");
  }
  // primary CTA opens the welcome screen — the editorial doorway to the menu
  const cta = $(".hero__cta");
  if (cta) cta.dataset.goto = story.getStory() ? "welcome" : (data.dishes[0]?.id ?? "drinks");
}

/* ---------- global category index (two taps from every screen) ---------- */
let menuIndexBound = false;

export function initMenuIndex() {
  if (menuIndexBound) return;
  const panel = $("#menu-index");
  const toggle = $("#menu-index-toggle");
  const closeButton = $("#menu-index-close");
  ...
  const setOpen = (open, restoreFocus = false) => {
    ...
    document.body.classList.toggle("menu-index-open", open);
    if (open) {
      requestAnimationFrame(() => $(".menu-index__link", panel)?.focus({ preventScroll: true }));
  ...
  panel.addEventListener("click", (event) => {
    if (event.target.closest("[data-goto]")) setOpen(false);
  });
...
function buildDish(dish, tpl, catLabel, favs) {
  ...
  $(".dish__texture", node).textContent = dish.texture ||
    (dish.subtitle?.en || "KRISTO").split(" ").reduce((a, b) => (b.length > a.length ? b : a));
...
function toggleFavorite(id, btn) {
  ...
  storage.set("kristo:favorites", [...favs]);
...
async function share(dish) {
  ...
  const payload = { title: `كريستو — ${t(dish.name)}`, text: t(dish.description), url };
  try {
    if (navigator.share) {
...
export function openSheet(dish, trigger) {
  ...
  linksEl.innerHTML = entries.length
    ? entries.map(([app, url]) =>
        `<a href="${url}" target="_blank" rel="noopener noreferrer">${appLabel(app)}</a>`).join("")
