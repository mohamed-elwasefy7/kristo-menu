  ...
  io = new IntersectionObserver(
    (entries) => {
      for (const entry of entries) {
        ...
        if (index === -1 || entry.target === announcedSection && !rebuilding) continue;
        setActive(index);
      }
    },
...
function setActive(index) {
  ...
  const nav = $("#nav");
  announcedSection = section;
  screens.forEach((screen) => {
    const isActive = screen === section;
    screen.classList.toggle("is-active-screen", isActive);
...
export function enhanceInput() {
  if (observer || prefersReducedMotion() || !window.Observer || !window.gsap) return;
...
function initWheelAndKeys() {
  enhanceInput();
  if (!observer && !prefersReducedMotion() && window.Observer) {
    gsap.registerPlugin(Observer);
    observer = Observer.create({
      target: container,
      type: "wheel",
...
export function initNavAutoHide() {
  const nav = $("#nav");
  // A hash jump may already have positioned the snap container before this
  // listener is attached, so use that restored position as the baseline.
  let lastTop = container.scrollTop;
  let ticking = false;
  let autoHideReady = false;

  // Hash navigation can complete before the scroll listener is attached.
  // Start visible and reveal once more after that initial jump settles.
  ...
  // Keep a debounce as a universal fallback: some browsers advertise
  // scrollend but skip it for instant hash/programmatic jumps.
  let t2;
  container.addEventListener("scroll", () => {
    clearTimeout(t2);
