...
def main():
    ...
    for px in (192, 512):
        em.resize((px, px), Image.LANCZOS).save(OUT / f"icon-{px}.png", "PNG")
    ...
    canvas.save(OUT / "icon-maskable-512.png", "PNG")

    em.resize((180, 180), Image.LANCZOS).save(OUT / "apple-touch-icon.png", "PNG")
